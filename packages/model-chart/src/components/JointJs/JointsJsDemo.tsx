...
paper.on("blank:pointerdown", function (evt: dia.Event) {
  evt.data = {
    clientX: evt.clientX,
    clientY: evt.clientY,
...
paper.on("blank:pointermove", function (evt: dia.Event) {
...
paper.on("node:button:pointerclick", function (nodeView: dia.ElementView) {
...
class Node extends dia.Element<NodeAttributes> {
  ...
  changeDataPoint(names: string[], values: any[]) {
...
class NodeView extends dia.ElementView {
  private vSeparator: Vectorizer | null;
  private vBody: Vectorizer;
  private vLabel: Vectorizer;
  private vButton: Vectorizer;
  private vCounterNames: Vectorizer[];
  private vCounterValues: Vectorizer[];
  private counterValuesCache: string[];
  ...
  confirmUpdate(flag: number, opt: any) {
  ...
  renderCounterGroup(): Vectorizer {
    const { model } = this;
    const { width } = model.size();
    const vCounterNames: Array<Vectorizer> = (this.vCounterNames = []);
    const vCounterValues: Array<Vectorizer> = (this.vCounterValues = []);
...
function getCellsFromUid(graph: dia.Graph, uid: string): Node[] {
  const uidMap = graph.get("uidMap");
  if (!uidMap) return [];
  const ids = uidMap[uid];
  if (!ids) return [];
  return ids.reduce((acc: Node[], id: string) => {
...
function generateCells(graph: dia.Graph, c: number, r: number) {
  graph.set(
    "nodes",
...
interface DataPointEvent {
  uid: string;
  name: string;
  [counterName: string]: number | string;
}

function changeDataPoint(graph: dia.Graph, dataPointEvent: DataPointEvent) {
...
function changeState(graph: dia.Graph, stateEvent: StateEvent) {
...
function runEvents(
  ...
  const id2 = setInterval(() => {
    ...
    let counterNames = element.get("counterNames") as Array<string>;
    ...
    const counterPairs = counterNames.reduce((acc: any, name) => {
...
function buildUidMap(graph: dia.Graph) {
  const prevUidMap = Object.assign({}, graph.get("uidMap"));
  const uidMap: { [id: string]: Array<string | number> } = {};
...
function enableVirtualRendering(paper: dia.Paper) {
